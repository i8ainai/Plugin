...
void UUDPComponent::BeginPlay()
{
	...
	{
		TryOpenReceiveSocketRepeatedly();
	}
}


void UUDPComponent::TryOpenReceiveSocketRepeatedly()
{
	// 每5秒检测一次，如果socket未打开则尝试重新绑定
	GetWorld()->GetTimerManager().SetTimer(CheckNetworkHandle, [this]() {
		if (!Native->Settings.bIsReceiveOpen)
		{
			bool bOpened = OpenReceiveSocket(Settings.ReceiveIP, Settings.ReceivePort);
			if (bOpened)
			{
				UE_LOG(LogTemp, Log, TEXT("UDP Receiver socket successfully bound."));
				// 绑定成功后停止定时器
				GetWorld()->GetTimerManager().ClearTimer(CheckNetworkHandle);
			}
			else
			{
				UE_LOG(LogTemp, Warning, TEXT("Retrying to bind UDP Receiver socket..."));
			}
		}
		}, 5.0f, true);
}
...
bool FUDPNative::OpenReceiveSocket(const FString& InListenIP /*= TEXT("0.0.0.0")*/, const int32 InListenPort /*= 3002*/)
{
	...
	if (!ReceiverSocket)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to create UDP ReceiverSocket on %s:%d"), *Settings.ReceiveIP, Settings.ReceivePort);
		return false;
	}
